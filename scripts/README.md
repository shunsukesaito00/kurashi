...
- `booth-config.mjs` — BOOTH 導線の必須ファイル一覧（`REQUIRED_BOOTH_FILES`）・`isRequiredBoothFile()` と共通ヘルパー
- `check-booth-links.mjs` — BOOTH 導線（`data-booth-url`）の設定状況。必須3ファイルの構造・URL チェック。必須外の空属性は WARN のみ（`--strict` / `BOOTH_URL_STRICT=1` は必須のみ FAIL）
...
- `set-booth-url.mjs` — BOOTH 商品URLを `data-booth-url` に一括設定（`booth-config.mjs` の必須3ファイルのみ更新。必須外は WARN）
