...
- `operator-checks.mjs` — 運営者ブロッカー用ヘルパー（`isOperatorInfoReady()`。BOOTH「出品準備中」表記で誤判定しない）。`PASTE_TEMPLATE_REFERENCE_LINE` は `operator-status.mjs` の CLI 出力と `operator-status.test.mjs` で共有し、README 貼付例節へのアンカー案内文言のずれを防ぐ
