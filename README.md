...
| 設置候補 | 文言例 |
|---|---|
| `tedori.html` 比較結果の下 | 「この比較をスプレッドシートで保存したい方はBOOTHで」 |
...
- [x] 計算式仕様を [`docs/spreadsheet-spec.md`](docs/spreadsheet-spec.md) に沿ってシート化 → [`products/tedori-kakei-template.xlsx`](products/tedori-kakei-template.xlsx)（手取り試算＋比較の2シート。`python3 scripts/generate-spreadsheet-template.py` で再生成可）
- [ ] 免責・概算である旨をREADMEシートに記載
- [ ] サムネイル（1280×1280、比較表のスクショ＋タイトル）
- [ ] BOOTHアカウント開設・本人確認
- [ ] 価格 980円・ダウンロード販売で出品
- [ ] サイトにBOOTHリンクを1箇所追加（出品URL確定後）
