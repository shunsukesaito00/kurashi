...
代表例を変えるときは、この表と `index.html` のデモリンク、`scripts/verify-share-urls.mjs` の各ケース `path` を同時に更新すること。更新後は `cd scripts && npm test` で代表例の同期確認・`test:booth`（BOOTH導線のユニット・CLIテスト）・BOOTH導線確認・アフィリエイト構造確認と共有URLの往復テストをまとめて検証できる（`test:share-urls` 用に別ターミナルで HTTP サーバーを起動したまま）。
...
代表例の同期確認と共有URLの往復テスト(`scripts/` 配下)は次のとおりです。`npm test` は `check:demo-sync` → `test:booth` → `check:booth-links` → `check:affiliate-sections` → `check:aff-placeholders` → `test:share-urls` の順で実行します（前者5つは HTTP サーバー不要、`test:share-urls` は別ターミナルで HTTP サーバーを起動したまま）。BOOTH 導線だけ先に検証したい場合は `cd scripts && npm run test:booth` でも可です。
...
`check:booth-links` は `test:booth` の直後に `npm test` で実行され、未設定時は **WARN のみ**（exit 0）。`BOOTH_URL_STRICT=1` を付けると未設定の `data-booth-url` があると FAIL します。
...
| アフィリエイト導線(UI) | **一部完了** — 積立・時給・手取りページにPR表記付き比較セクションと広告枠(コード未設置) |
| 収益 | **0円** — ASP未登録・広告コード未設置・AdSense未申請 |
| 運営者作業(律速) | **一部完了** — about 実名義済み・A8/Search Console 手順・コピペ文済み。**未完了: A8.net登録・広告コード6枠・Search Console確認** |
...
| 設置候補 | 文言例 |
|---|---|
| `tedori.html` 比較結果の下 | 「この比較をスプレッドシートで保存したい方はBOOTHで」 → `data-booth-url` プレースホルダ設置済み（出品URL確定後に属性を更新） |
| `index.html` フッター付近 | 「有料テンプレート（BOOTH）」リンク → `data-booth-url` プレースホルダ設置済み |
| 共有URL利用者 | ツールは再現できるが、家計全体は有料版 |

BOOTH URL は出品後に `node scripts/set-booth-url.mjs --url <商品URL>` で `about.html`・`index.html`・`tools/tedori.html` を一括更新する。
...
- [ ] BOOTHアカウント開設・本人確認
- [ ] 価格 980円・ダウンロード販売で出品
- [x] サイトにBOOTHリンクを1箇所追加（出品URL確定後）→ `set-booth-url.mjs --url <商品URL>` で `index.html`・`tedori.html` を一括設定 → `BOOTH_URL_STRICT=1 npm test` で導線チェックを厳格化
