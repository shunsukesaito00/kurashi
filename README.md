...
代表例を変えるときは、この表と `index.html` のデモリンク、`scripts/verify-share-urls.mjs` の各ケース `path` を同時に更新すること。更新後は `cd scripts && npm test` で代表例の同期確認・BOOTH導線確認・アフィリエイト構造確認と共有URLの往復テストをまとめて検証できる（`test:share-urls` 用に別ターミナルで HTTP サーバーを起動したまま）。
...
`check:booth-links` は通常 `npm test` に含まれ、未設定時は **WARN のみ**（exit 0）。`BOOTH_URL_STRICT=1` を付けると未設定の `data-booth-url` があると FAIL します。
...
BOOTH URL は出品後に `about.html` または該当ツールに1リンク追加する。
...
- [x] ZIP納品物（xlsx + PDFマニュアル）→ [`products/tedori-kakei-booth.zip`](products/tedori-kakei-booth.zip)（`python3 scripts/build-booth-package.py` で再生成可）
- [x] 免責・概算である旨をREADMEシートに記載 → `products/tedori-kakei-template.xlsx` の README シート
- [ ] サムネイル（1280×1280、比較表のスクショ＋タイトル）
- [ ] BOOTHアカウント開設・本人確認
- [ ] 価格 980円・ダウンロード販売で出品
